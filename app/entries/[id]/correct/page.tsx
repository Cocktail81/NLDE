...
interface OriginalEntry extends LaundryItemQuantities {
  id: string
  entry_date: string
  customer_id: string
...
export default function CorrectEntryPage() {
  ...
  const fetchOriginalEntry = useCallback(async () => {
    ...
    const { data: requestedEntry, error: entryError } = await supabase
      .from('entries')
      .select('*')
      .eq('id', entryId)
      .single()
  
    if (entryError || !requestedEntry) {
    ...
    if (!requestedEntry.is_current_version) {
      const { data: currentEntry, error: currentEntryError } = await supabase
        .from('entries')
        .select('*')
        .eq('customer_id', requestedEntry.customer_id)
        .eq('entry_date', requestedEntry.entry_date)
        .eq('is_current_version', true)
        .single()
  
      if (currentEntryError || !currentEntry) {
        setError('Current version of this entry was not found')
        setFetching(false)
        return
      }
    ...
    const originalItemQuantities = LAUNDRY_ITEMS.reduce((acc, item) => {
    ...
    setOriginalEntry({
      ...
      total_items: calculateLaundryTotal(originalItemQuantities),
      is_current_version: entry.is_current_version || false,
    })
    ...
    setItemQuantities(
      LAUNDRY_ITEMS.reduce((acc, item) => {
        acc[item.key] = String(entry[item.key] || 0)
        return acc
      }, {} as Record<LaundryItemKey, string>)
    )
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!originalEntry) {
      setError('Original entry not loaded')
      return
    }
    ...
    try {
      ...
      const correctionData = {
        ...
        is_correction: true,
        previous_version_id: originalEntry.id,
  ...
  const hasChanges = () => {
    ...
    return (
      ...
      LAUNDRY_ITEMS.some(item => {
        return (
          (Number.parseInt(itemQuantities[item.key], 10) || 0) !==
          originalEntry[item.key]
        )
      })
    )
  }
          ...
          <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-xl">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 text-sm">
              ...
              {LAUNDRY_ITEMS.map(item => (
                <div key={item.key}>
                  <p className="text-yellow-600">{item.shortLabel}</p>
