...
export default function CustomerReport({
  ...
  const getCustomerSummary = () => {
    const totalIroning = customerEntries.reduce((sum, e) => sum + e.ironing, 0)
    const totalSaree = customerEntries.reduce((sum, e) => sum + e.saree_ironing, 0)
    const totalDryClean = customerEntries.reduce((sum, e) => sum + e.dry_cleaning, 0)
    const totalItems = customerEntries.reduce((sum, e) => sum + e.total, 0)
    return { totalIroning, totalSaree, totalDryClean, totalItems, totalEntries: customerEntries.length }
  }
    ...
    <div className="overflow-x-auto">
      ...
      >
        ...
        <thead className="bg-gray-50 print:bg-gray-100">
          <tr>
            <th className="py-2 px-1 text-center font-semibold text-xs print:text-[6.5pt]">Date</th>
            <th className="py-2 px-1 text-center font-semibold text-xs print:text-[6.5pt]">Ironing</th>
            <th className="py-2 px-1 text-center font-semibold text-xs print:text-[6.5pt]">Saree</th>
            <th className="py-2 px-1 text-center font-semibold text-xs print:text-[6.5pt]">Dry Clean</th>
            <th className="py-2 px-1 text-center font-semibold text-xs print:text-[6.5pt]">Total</th>
          </tr>
        </thead>
        ...
        <tbody>
          {customerEntries.map((entry) => (
            <tr key={entry.id} className="hover:bg-gray-50 print:hover:bg-transparent">
              ...
              <td className="py-1.5 px-1 text-center tabular-nums text-xs print:text-[6pt]">
                {entry.ironing}
              </td>
              <td className="py-1.5 px-1 text-center tabular-nums text-xs print:text-[6pt]">
                {entry.saree_ironing}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border p-6 print:hidden">
        <div className="mb-4">
          <h3 className="text-lg font-semibold text-gray-800 mb-2">Customer Report</h3>
          <p className="text-sm text-gray-500">Select a customer and date range to view their entries</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-center justify-between">
          <p className="text-sm text-gray-500">
            {selectedCustomer ? (
              <>
                Showing entries for <span className="font-medium text-gray-700">{getCustomerName()}</span> from{' '}
        ...
        {customerEntries.length > 0 ? (
          <div className="bg-white border overflow-hidden print:border-none">
            ...
            <div className="p-4 border-b text-center print:hidden">
              ...
              <p className="text-xl font-bold text-green-600 mt-1">{getCustomerName()}</p>
            ...
            <div className="p-4 print:p-2">
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-4">
                <div className="text-center p-3 bg-gray-50 rounded-lg print:border print:border-gray-300">
                  <div className="text-2xl font-bold text-gray-900 print:text-base">{summary.totalEntries}</div>
                  <div className="text-xs text-gray-600">Entries</div>
                </div>
                <div className="text-center p-3 bg-gray-50 rounded-lg print:border print:border-gray-300">
                  <div className="text-2xl font-bold text-gray-900 print:text-base">{summary.totalIroning}</div>
                  <div className="text-xs text-gray-600">Ironing</div>
                </div>
                <div className="text-center p-3 bg-gray-50 rounded-lg print:border print:border-gray-300">
                  <div className="text-2xl font-bold text-gray-900 print:text-base">{summary.totalSaree}</div>
                  <div className="text-xs text-gray-600">Saree</div>
                </div>
                <div className="text-center p-3 bg-gray-50 rounded-lg print:border print:border-gray-300">
                  <div className="text-2xl font-bold text-gray-900 print:text-base">{summary.totalDryClean}</div>
                ...
                <div className="text-center p-3 bg-blue-50 rounded-lg print:border print:border-gray-300">
                  <div className="text-2xl font-bold text-blue-700 print:text-base">{summary.totalItems}</div>
            ...
            <div className="p-3 bg-gray-50 border-t print:bg-gray-100">
              <div className="flex flex-col sm:flex-row justify-between items-center text-sm">
                <span className="text-gray-600">
                  Total Entries: <strong>{summary.totalEntries}</strong> | 
